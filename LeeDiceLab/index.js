...
function whoWon() {
  ...
  document.querySelector("h4").innerHTML = "Current balance: " + balance;
...
function letsPlay(){
  numRolls = prompt("How many times do you want to roll?");
  let x = 0;
  for (let i = 0; i < numRolls; i++) {
    const timeout = setTimeout(rollDice, 1000 * (i + 1));
    const timeout2 = setTimeout(whoWon, 1000 * (i + 1));
    x = i + 1
  }
  balance = 0;
  const timeout3 = setTimeout(buttonChange, x * 1000);
}
